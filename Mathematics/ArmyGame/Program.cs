...
namespace ArmyGame
{
    class Program
    {
        ...
        // a single drop off can cover at most 4 cells (if arranged in a 2*2 grid),
        // otherwise at most 2 cells if arranged horizontally or vertically
        static int gameWithCells(int n, int m)
        {
            ...
            if (rows % 2 != 0) { // drop offs for an additional row
                extraRow = (int) Math.Ceiling(columns / 2M);
            }

            if (columns % 2 != 0) { // drop offs for an additional column
